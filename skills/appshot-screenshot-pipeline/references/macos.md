...
Three app-side requirements, all behind the demo flag. The first is to *not* activate: skip the app's own `NSApplication.activate` when `-ScreenshotActivation` is `none`, or the run that promises not to take the screen takes it at every launch. The other two:
